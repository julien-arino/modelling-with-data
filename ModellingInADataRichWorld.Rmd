...
output:
  pdf_document:
    includes:
      in_header: latexHeaderElsart.tex
      before_body: latexBeforeBody.tex
    fig_caption: true
    number_sections: yes
    keep_tex: yes
...
if (!DOWNLOAD) {
  ...
  GHO_indices <- readRDS("DATA/GHO_indices.Rds")
...
```{r print_table} 
kable(GHO_indices[idx_polio,1:2], row.names = FALSE, format = "latex", booktabs = TRUE) %>% kable_styling(latex_options = "striped")
```

Using the name of one of the indices in the table above (e.g., `poliov`) as `indicator` in the first chunk in this section would then allow to load the corresponding dataset.

### Using existing `R` libraries{#sec:R_API_libs}
`R` abounds with packages allowing to perform easily the operations I carried out with the WHO GHO indices in Section \ref{sec:usingAPI}.
...
```{r air_transport, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="\\label{fig:air_transport} Evolution of the annual number of passengers transported by air transport and of the number of aircraft movements.",fig.subcap=c('Passengers transported','Aircraft movements'),out.width='.49\\linewidth'}
if (DOWNLOAD) {
...
```{r,echo=FALSE,fig.cap="\\label{fig:influenza_FR_data} Weekly number of cases of influenza in France as reported by the Réseau Sentinelles. (a) All dates. (b) 2018-2019 epidemic season.",fig.subcap=c('\\label{fig:influenza_FR_data_allDates}All dates.', '\\label{fig:influenza_FR_data_lastYear}2018-2019.'),out.width='.49\\linewidth'}
...
```{r,echo=FALSE,fig.cap="\\label{fig:influenza_FR_data_lastYear_regions} Weekly number of cases of influenza in each of the 13 regions of metropolitan France for the 2018-2019 epidemic season, as reported by the Réseau Sentinelles. Numbers are per 100,000."}
