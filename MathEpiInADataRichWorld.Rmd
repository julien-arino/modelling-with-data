...
This is a basic formatting of the paper "Mathematical epidemiology in a data-rich world", that was published in early 2020 and is accessible [here](https://doi.org/10.1016/j.idm.2019.12.008). 
There were some issues with formatting in the published version. In particular, image files were generated that should have been code blocks. Also, some of the code I used at the time is no longer running: until all web available data gets persistent identifiers (such as doi for papers), this type of paper is doomed to run into issues, with data being moved or formats being changed. 

In the present version, I have updated the code so that it runs. 
I have also slightly adapted the code: since version 4.0 of `R`, data frames automatically are created with `stringsAsFactors = FALSE`, so this command is not required anymore.
Besides these changes, the content of the paper is still exactly the same as the published version.
