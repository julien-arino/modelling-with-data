...
This is a low level formatting of the paper "Mathematical epidemiology in a data-rich world", that was published in early 2020. There were some issues with formatting, Elsevier generated image files where there should have been code blocks. Also, some of the code I used at the time is no longer running: until all web available data gets persistent identifiers (such as doi for papers), this type of paper is doomed to run into issues. In the present version, I have updated the code so that it still runs. The content of the paper is still exactly the same as the published version, though.
...
if (DOWNLOAD) {
  ...
  pop_data_CHN_wb <- readRDS("DATA/pop_data_CHN_wb.Rds")
  colnames(pop_data_CHN_wb)[5] <- "value"
}
